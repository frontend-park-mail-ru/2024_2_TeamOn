...
import { renderUserPost } from "../../entities/userPost/ui/ui";
...
if (buttonSave) {
  buttonSave.addEventListener("click", async () => {
    const title: HTMLInputElement | null =
      containerCreatePost?.querySelector(`.input-group`);
    const content: HTMLTextAreaElement | null =
      containerCreatePost?.querySelector(`.textarea-group`);
    ...
    try {
      ...
      if (placeStats) {
        const statsVNode = await renderUserStats(
          {
            /* authorData */
          },
          payments,
