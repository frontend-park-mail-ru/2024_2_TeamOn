import { users, maxAttempts, state } from "./consts.js";
...
function addUser(username, email = null, password, imagesrc = null) {
  // tmp
  users[username] = {
    email,
    password,
...
function renderSignup() {
  ...
  function validateForm() {
    ...
    if (
      ...
      passwordErrors.push(
        "Пароль должен быть не менее 8 и не более 64 символов",
      );
    }
    ...
    if (
      !/(?=.*[a-z])(?=.*[A-Z])/.test(DOMPurify.sanitize(inputPassword.value))
    ) {
      passwordErrors.push(
